...
\begin{figure}[htbp]
    \begin{center}
        \begin{tikzpicture} [>=latex', node distance = 2cm]
            ...
            \draw[->](resolver.east) ++(0,-0.25) -- ++(1.5,0) node[midway,above]{load};
...
inside the single layer model can be recduced. In the concrete hardware model, the 9-Bit
address line go through all the layer resolvers, which split them in a 5-Bit address line, which
is connected to every perceptron in the particular layer. The data line stay a 5-Bit and the load
